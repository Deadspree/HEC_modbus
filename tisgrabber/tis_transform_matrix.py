...
def new_charuco_pose_estimation(image, cameraMatrix, distCoeffs, board):
    """
    ...
    immarkers = cv2.aruco.drawDetectedMarkers(
        image.copy(), marker_corners, marker_ids
    )
    ...
    if charuco_ids is not None and len(charuco_corners) > 0:
        ...
        if len(objPoints) >= 20:
            ...
            """
            # Try different method for solvepnp
            cv2.drawFrameAxes(
...
def calculate_T_cam_charuco(image: np.ndarray, board) -> np.ndarray:
    """
...
def pose_estimation(
    ...
    @matrix_coefficients_path (str): Path to load the matrix_coefficients
    from camera calibration
    @distortion_coefficients_path (str): Path to load the
    distortion_coefficients from camera calibration
...
def main():
    ...
    if ic.IC_IsDevValid(hGrabber):
        ...
        # Set manual values (adjust these based on your lighting)
        # ic.IC_SetPropertyValue(hGrabber, tis.T("Exposure"), tis.T("Value"), 0)  # Start with -5 to -7
        # ic.IC_SetPropertyValue(hGrabber, tis.T("Gain"), tis.T("Value"), 0)  # Keep gain low, 0-10
        ic.IC_StartLive(hGrabber, 0)
        board = cv2.aruco.CharucoBoard(
            (7, 5), 0.03, 0.022, cv2.aruco.getPredefinedDictionary(
        ...
        while True:
            if ic.IC_SnapImage(hGrabber, 2000) == tis.IC_SUCCESS:
                ...
                matrix_coefficients_path = (
                    PROJECT_ROOT / "input" / "calibration_matrix.npy"
                )
                distortion_coefficients_path = (
                    PROJECT_ROOT / "input" / "distortion_coefficients.npy"
                )
