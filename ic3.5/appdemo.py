...
sys.path.append(os.getenv('IC35PATH') + r"/redist/dotnet/x64")
...
class MainWindow(QMainWindow):
    ...
    def keyPressEvent(self, event):
        ...
        if key == Qt.Key_S:
            print("S pressed — capturing frame")
            SnapImage()
...
def SnapImage():
    ...
    # Save directory
    save_dir = PROJECT_ROOT / "camera_cal"
    save_dir.mkdir(parents=True, exist_ok=True)

    # Auto-increment filename
    filename = save_dir / f"calib_{image_counter:03d}.jpg"

    # Save JPEG
    TIS.Imaging.FrameExtensions.SaveAsJpeg(image, str(filename), 100)

    print(f"Saved {filename}")

    # Increase counter for next frame
    image_counter += 1
