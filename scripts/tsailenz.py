...
def calibrate_eye_hand(T_base_ee_list: list[np.ndarray], T_cam_marker_list: list[np.ndarray], cal_val: int, eye_to_hand: bool=True) -> np.ndarray:
    ...
    t_ee2base = [T[:3, 3] for T in T_base_ee_list]

    R_marker2cam = [T[:3, :3] for T in T_cam_marker_list]
    t_marker2cam = [T[:3, :3] for T in T_cam_marker_list]


    if eye_to_hand:
        # change coordinates from ee/gripper2base to base2gripper/ee
    ...
    trans_errors = []
    rot_errors = []
    for i in range(cal_val,len(T_base_ee_list)-1):
...
def main():
    ...
    JSON_PATH = PROJECT_ROOT / "data" / "transforms1.json"
    ...
    T_base_cam = calibrate_eye_hand(T_base_ee_list, T_cam_marker_list, 10)
